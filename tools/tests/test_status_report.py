...
def test_render_progress_precedence_order():
    ...
    assert python_mark == "✓"  # pass beats n/a
...
@pytest.mark.parametrize("other", ["not-run", "skip"])
def test_render_progress_partial_mark(other):
    """Ruling S21: pass mixed with not-run/skip over one runner's flows is partial (◐),
    display-only; fail and error still dominate; pass with unsupported/n/a stays ✓."""
    prim = _status(
        "FDRE",
        results={
            "L1/iverilog/rtl": "pass",
            "L1/iverilog/vivado": other,
            "L1/xsim/rtl": "pass",
            "L1/xsim/vivado": other,
            "L1/xsim/yosys": "fail",
            "L1/python/rtl": "pass",
            "L1/python/x": "unsupported",
        },
    )
    out = render_progress([prim], _unit(primitives=("FDRE",)))
    row = next(line for line in out.splitlines() if line.startswith("| FDRE "))
