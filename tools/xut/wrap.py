...
def render_cocotb_top(m: DutMap) -> str:
    ...
    resolves upward to this instance (IEEE 1364 upward name referencing). ``clk`` and
    ``in_vec`` are variables that start at 0, not top-level input ports, which would float
    (z) until cocotb's first write: every input is driven from time 0, as the z-compare
    rewrite requires (ruling S38, review M3)."""
    return (
        HEADER + f"// GENERATED by xut wrap --cocotb-top: prim={m.prim} cfg={m.cfg}. Do not edit.\n"
        '`timescale 1ps / 1ps\n`include "xut_cfg.vh"\n'
        "module xut_cocotb_top;\n"
        "  reg  [`XUT_NCLK-1:0] clk = {`XUT_NCLK{1'b0}};\n"
        "  reg  [`XUT_NIN-1:0]  in_vec = {`XUT_NIN{1'b0}};\n"
