...
def catalog_build_cmd() -> None:
    ...
    from xut.catalog.build import FAMILY, build_all
    ...
    lines = [
        ...
        "",
        f"Generated by `xut catalog build` from {UG953.name.upper()} {UG953.edition} and the "
        f"UNISIM models in {', '.join(s.name + '/' for s in search)}.",
        "Resolve each item with a `<PRIM>.overrides.yaml` or accept it as a documented",
        "doc-vs-model difference.",
        "",
        f"- primitives: {len(names)}",
        f"- report lines: {len(report)}",
        f"- names only in one source (`only in`): {only_in}",
        "",
        *(f"- {line}" for line in report),
        "",
    ]
    (root / "catalog" / "EXTRACTION_REPORT.md").write_text("\n".join(lines))
    click.echo(
